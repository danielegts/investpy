...
from investpy.quotes import get_quotes
from .data.stocks_data import (
	stocks_as_df,
	stocks_as_dict,
	stocks_as_list,
)
from .globals import PROXIES
...
def get_stocks_dict(country=None, columns=None, as_json=False):
	...
	Args:
		country (:obj:`str`, optional): name of the country to retrieve all its available stocks from.
		columns (:obj:`list`, optional):column names of the stock data to retrieve, can be: <country, name, full_name,
		isin, currency, symbol>
		as_json (:obj:`bool`, optional): if True the returned data will be a :obj:`json` object, if False,
		a :obj:`list` of :obj:`dict`.

	Returns:
		:obj:`list` of :obj:`dict` OR :obj:`json` - stocks_dict:
			The resulting :obj:`list` of :obj:`dict` contains the retrieved data from every stock as indexed in
			Investing.com from
...
def get_stock_recent_data(
	...
	:obj:`json` or not, and if the historical data is going to be ordered ascending or descending (where the index is
	the
	date), respectively.
	...
	Args:
		...
		as_json (:obj:`bool`, optional):
			to determine the format of the output data, either a :obj:`pandas.DataFrame` if False and a :obj:`json` if
			True.
		order (:obj:`str`, optional): to define the order of the retrieved data which can either be ascending or
		descending.
		interval (:obj:`str`, optional):
			value to define the historical data interval to retrieve, by default `Daily`, but it can also be `Weekly`
			or `Monthly`.
	...
	Returns:
		:obj:`pandas.DataFrame` or :obj:`json`:
			...
			open, high, low, close and volume values for the selected stock on market days and the currency in which
			those
...
def get_stock_historical_data(
	...
	be returned as a :obj:`json` or not, and if the historical data is going to be ordered ascending or descending (
	where the
	...
	Returns:
		:obj:`pandas.DataFrame` or :obj:`json`:
			The function can return either a :obj:`pandas.DataFrame` or a :obj:`json` object, containing the retrieved
			historical data of the specified stock from the specified country. So on, the resulting dataframe contains
			the
			open, high, low, close and volume values for the selected stock on market days and the currency in which
			those
			values are presented.
	...
	src_res = search_quotes(stock, ['stocks'], [country])

	if len(src_res) > 0:
		return get_quotes(src_res[0].id_, start_date, end_date, order, interval, as_json, head)
	else:
		raise IndexError(
			"ERR#0069: product information unavailable or not found."
		)


def get_stock_company_profile(stock, country="united states", language="english"):
	"""
	...
	Args:
		...
		country (:obj:`str`): name of the country from where the stock is.
		language (:obj:`str`, optional): language in which the company profile is going to be retrieved, can either be
		english or spanish.
	...
	if path_:
		...
		if path_:
			...
			while flag is True:
				...
				req = requests.post(url=url, headers=headers, params=params, proxies=PROXIES)
...
def get_stocks_overview(country, as_json=False, n_results=100):
	...
	Raises:
		...
		RuntimeError:
			raised either if the introduced country does not match any of the listed ones or if no overview results
			could be
	...
	req = requests.get(url, params=params, headers=head, proxies=PROXIES)
...
def get_stock_financial_summary(
	...
	Examples:
		>>> data = investpy.get_stock_financial_summary(stock='AAPL', country='United States',
...
def search_stocks(by, value):
	...
	Args:
		by (:obj:`str`): name of the field to search for, which is the column name which can be: 'name', 'full_name'
		or 'isin'.
