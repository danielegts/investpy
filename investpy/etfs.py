...
def get_etfs(country=None):
	...
	Returns:
		:obj:`pandas.DataFrame` - etfs:
			The resulting :obj:`pandas.DataFrame` contains all the etfs basic information stored on `etfs.csv`,
			since it
...
def get_etf_countries():
	...
	Returns:
		:obj:`list` - countries:
			...
			In the case that the file reading of `etf_countries.csv` which contains the names and codes of the
			countries
...
def get_etf_historical_data(
	...
	Examples:
		>>> data = investpy.get_etf_historical_data(etf='bbva accion dj eurostoxx 50', country='spain',
		from_date='01/01/2010', to_date='01/01/2019')
		>>> data.head()
					 Open   High    Low  Close  Volume Currency Exchange
		Date
	...
	if etf not in list(etfs["symbol"].apply(unidecode).str.lower()):
		raise RuntimeError(
			"ERR#0019: etf " + etf + " not found, check if it is correct."
		)

	etfs = etfs[etfs["symbol"].apply(unidecode).str.lower() == etf]
	...
	src_res = search_quotes(etf, ['etfs'], [country])
